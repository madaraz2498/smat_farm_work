  ...
  bool _validate() {
    ...
    setState(() {
      ...
      if (_nameCtrl.text.trim().isEmpty) {
        ...
        _nameError = 'Name must be at least 2 characters.'; ok = false;
      }

      final email = _emailCtrl.text.trim();
      if (email.isEmpty) {
        _emailError = 'Email is required.'; ok = false;
      } else if (!RegExp(r'^[\w.-]+@[\w.-]+\.\w+$').hasMatch(email)) {
        _emailError = 'Enter a valid email address.'; ok = false;
      }

      final pass = _passCtrl.text;
      if (pass.isEmpty) {
        _passError = 'Password is required.'; ok = false;
      } else if (pass.length < 6) {
        _passError = 'Password must be at least 6 characters.'; ok = false;
      }

      if (_confirmCtrl.text.isEmpty) {
        _confirmError = 'Please confirm your password.'; ok = false;
